...
int main(){
    ...
    for(unsigned char i=0;i<nTxt;i++){ //CANTIDAD DE TXTs
        ...
        cin >> rutaInputTxt;
        const char* rutaTxt = rutaInputTxt.c_str();
        unsigned int* ptrTxt = loadSeedMasking(rutaTxt, seed, nPixeles); // apuntador hacia datos txt // (primer parametro verificar)
        restarArreglos(ptrTxt, ptrM, nPixeles);
        transformacion = identificarTransformacion(ptrTxt,ptrI_D,seed);
        transformacionInversa(ptrI_D,transformacion);
        delete ptrTxt;
    }
